<x-admin-app>
    <x-slot name="pageTitle">Edit Metode Pembayaran & Rekening</x-slot>
    <x-slot name="breadcrumb">Perbarui informasi rekening dan instruksi pembayaran</x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="glass rounded-3xl p-6 sm:p-8 bg-white border border-[#e8e3d5] shadow-lg">
            <div class="flex items-center gap-3.5 mb-6 pb-4 border-b border-slate-200">
                ...
                <div>
                    <h2 class="text-xl font-bold font-playfair text-slate-900">Edit Rekening: {{ $paymentMethod->name }}</h2>
                    <p class="text-xs text-slate-500 font-medium">Ubah nomor rekening, nama pemilik, atau instruksi pembayaran.</p>
                </div>
            </div>
            ...
            <form action="{{ route('admin.payment-methods.update', $paymentMethod) }}" method="POST" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $paymentMethod->name) }}" required class="input-field font-bold">
                    </div>
                    <div>
                        <label class="input-label">Kode Unik <span class="text-red-600">*</span></label>
                        <input type="text" name="code" value="{{ old('code', $paymentMethod->code) }}" required class="input-field font-mono">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type" required class="input-field font-bold cursor-pointer">
                            ...
                            <option value="cash" {{ old('type', $paymentMethod->type) == 'cash' ? 'selected' : '' }}>💵 Tunai di Toko (Cash)</option>
                            <option value="qris" {{ old('type', $paymentMethod->type) == 'qris' ? 'selected' : '' }}>📱 QRIS / E-Wallet</option>
                            <option value="debit" {{ old('type', $paymentMethod->type) == 'debit' ? 'selected' : '' }}>💳 Kartu Debit</option>
                            <option value="credit" {{ old('type', $paymentMethod->type) == 'credit' ? 'selected' : '' }}>💳 Kartu Kredit</option>
                        </select>
                    </div>
                    <div>
                        <label class="input-label">Nama Bank / Institusi</label>
                        <input type="text" name="bank_name" value="{{ old('bank_name', $paymentMethod->bank_name) }}" class="input-field">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5 rounded-2xl bg-[#F4EDD9]/40 border border-[#e8e3d5]">
                    <div>
                        ...
                        <input type="text" name="account_number" value="{{ old('account_number', $paymentMethod->account_number) }}" class="input-field font-mono font-bold text-slate-900">
                    ...
                    <div>
                        ...
                        <input type="text" name="account_name" value="{{ old('account_name', $paymentMethod->account_name) }}" class="input-field font-bold text-slate-900">
                    </div>
                </div>

                <div>
                    <label class="input-label">Instruksi / Catatan Pembayaran</label>
                    <textarea name="instructions" rows="3" class="input-field">{{ old('instructions', $paymentMethod->instructions) }}</textarea>
                </div>
