...
<div class="max-w-4xl mx-auto flex flex-col justify-center items-center w-full">
    <div class="w-full mb-4 flex justify-between items-center">
        <a href="{{ route('admin.transactions.index') }}" class="text-xs font-bold text-[#085C54] hover:underline transition">← Kembali ke Daftar Transaksi</a>
    </div>

    @if(session('error'))
    <div class="w-full mb-4 p-4 rounded-xl text-sm font-bold text-red-900 bg-red-50 border border-red-300 shadow-sm" data-flash>❌ {{ session('error') }}</div>
    @endif
    @if($errors->any())
    <div class="w-full mb-4 p-4 rounded-xl text-sm font-bold text-red-900 bg-red-50 border border-red-300 shadow-sm" data-flash>
    ...
    @if($agreedPriceTotal)
    <div class="w-full rounded-2xl p-4 mb-4 bg-amber-50 border border-amber-300 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <span class="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-xs font-extrabold bg-amber-100 text-amber-900 border border-amber-300">
                🤝 Transaksi Hasil Tawar Harga Disetujui
            </span>
            <p class="text-sm font-bold text-slate-900 mt-1">Reservasi #{{ $selectedReservation->reservation_code }} ({{ $selectedReservation->user->name }})</p>
        </div>
        <div class="text-left sm:text-right">
            <p class="text-xs text-slate-500 font-bold uppercase tracking-wider">Total Kesepakatan</p>
            <p class="text-xl font-extrabold text-[#C6A443]">Rp {{ number_format($agreedPriceTotal, 0, ',', '.') }}</p>
        </div>
    </div>
    @endif
    ...
    <form method="POST" action="{{ route('admin.transactions.store') }}" id="trx-form" onsubmit="if(this.dataset.submitted) return false; this.dataset.submitted = true;" class="w-full">
        ...
        <div class="space-y-6">
            ...
            <div class="glass rounded-2xl p-6 shadow-md bg-white border border-[#e8e3d5]">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="reservation_id" id="reservation_select" class="input-field cursor-pointer">
                            ...
                            <option value="{{ $res->id }}" {{ old('reservation_id', $selectedReservation?->id) == $res->id ? 'selected':'' }}>
                                {{ $res->reservation_code }} — {{ $res->user->name }} — {{ $res->product->name ?? ($res->pawn_gold_description ?? 'Gadai/Jual Emas') }}
                                @if($res->agreed_price || $res->priceNegotiation)
                                (Tawar: Rp {{ number_format($res->agreed_price ?? $res->priceNegotiation->agreed_price, 0, ',', '.') }})
                                @endif
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label class="input-label">Pelanggan <span class="text-red-600">*</span></label>
                        <select name="user_id" class="input-field cursor-pointer" required>
                    ...
                    <div>
                        <label class="input-label">Tipe Transaksi <span class="text-red-600">*</span></label>
                        <select name="type" id="transaction_type" class="input-field cursor-pointer font-bold" required>
                            @foreach(['purchase'=>'Pembelian Emas (Tunai)','buyback'=>'Jual Emas ke Toko (Buyback)','installment'=>'Cicilan Emas','pawn'=>'Gadai Emas'] as $val=>$lbl)
                    ...
                    <div>
                        ...
                        <select name="gold_price_id" class="input-field cursor-pointer">
                    ...
                    <div>
                        <label class="input-label">Tanggal Transaksi / Bayar <span class="text-red-600">*</span></label>
                    ...
                    <div class="col-span-1 sm:col-span-2">
                        ...
                        <textarea name="notes" rows="2" class="input-field text-sm" placeholder="Opsional...">{{ old('notes', $selectedReservation?->notes) }}</textarea>
                    </div>
            ...
            {{-- Items Produk --}}
            <div class="glass rounded-2xl p-6 shadow-md bg-white border border-[#e8e3d5]" id="items-section">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-bold text-slate-900 flex items-center gap-2 text-base">
                        <span>📦</span> Item Produk Transaksi
                    </h3>
                    <button type="button" class="px-3.5 py-1.5 rounded-xl text-xs font-bold text-[#085C54] bg-[#e2f2f0] hover:bg-[#c9e8e4] border border-[#085C54]/20 transition shadow-sm" id="add-item-btn">+ Tambah Item</button>
                </div>
                <div id="items-container" class="space-y-3">
                    <div class="item-row grid grid-cols-12 gap-3 items-end p-3.5 rounded-xl bg-slate-50 border border-slate-200">
                        <div class="col-span-5">
                            <label class="input-label">Produk <span class="text-red-600">*</span></label>
                            <select name="items[0][product_id]" class="input-field item-product cursor-pointer" required onchange="fillPrice(this, 0)">
            ...
            <div class="glass rounded-2xl p-6 shadow-md bg-white border border-[#e8e3d5]">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div class="rounded-xl p-4 flex flex-col justify-center bg-amber-50 border border-amber-200 shadow-sm">
                        ...
                        <p class="text-2xl font-extrabold text-[#C6A443]" id="total-display">Rp 0</p>
                    </div>
                </div>
            </div>

            <div class="flex gap-4 pt-2">
                <button type="submit" class="flex-1 py-3 rounded-xl font-extrabold text-sm text-[#042623] gold-gradient border border-[#C6A443] shadow-lg hover:brightness-110 transition">
                    💾 Simpan Transaksi & Selesaikan
                </button>
                <a href="{{ route('admin.transactions.index') }}" class="px-6 py-3 rounded-xl text-sm font-bold text-slate-700 bg-white border border-slate-300 hover:bg-slate-50 transition flex items-center">Batal</a>
            </div>
        </div>
    </form>
</div>
