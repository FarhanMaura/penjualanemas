...
<head>
    ...
    <style>
        ...
        .sidebar-link { transition: color 0.15s ease, background-color 0.15s ease, border-color 0.15s ease; color: #475569; font-weight: 600; }
    ...
    <aside id="sidebar" class="sidebar w-64 flex flex-col fixed z-30 transition-transform duration-300 lg:translate-x-0"
        ...
        <div class="p-5 flex items-center gap-3" style="border-bottom:1px solid rgba(234,179,8,0.2);">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Panel Admin</p>
            </div>
            <!-- Close button (mobile) -->
        ...
        <nav class="flex-1 py-4 overflow-y-auto text-sm">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="sidebar-link w-full flex items-center gap-3 px-5 py-2.5 text-red-600 hover:text-red-700 hover:bg-red-50">
    ...
    <main class="flex-1 min-w-0 w-full p-4 lg:p-8 min-h-screen flex flex-col ml-0 lg:ml-64">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 lg:mb-8">
            <div class="flex items-center gap-3">
                ...
                <div>
                    @isset($pageTitle)
                        <h1 class="text-xl lg:text-2xl font-bold text-slate-900" style="font-family:'Playfair Display',serif;">{{ $pageTitle }}</h1>
                    @endisset
                    @isset($breadcrumb)
                        <p class="text-xs lg:text-sm text-slate-500 mt-1">{{ $breadcrumb }}</p>
                    @else
                        <p class="text-xs lg:text-sm text-slate-500 mt-1">{{ now()->isoFormat('dddd, D MMMM Y') }} • {{ now()->format('H:i') }} WIB</p>
                    @endisset
                </div>
            </div>
            <div class="flex items-center gap-3 lg:gap-4">
                ...
                <div class="flex items-center gap-2 lg:gap-3 glass px-3 lg:px-4 py-2 rounded-xl border border-amber-300">
                    ...
                    <div class="hidden sm:block">
                        ...
                        <p class="text-[10px] text-amber-800 font-semibold">Administrator</p>
                    </div>
                </div>
            </div>
        </div>
